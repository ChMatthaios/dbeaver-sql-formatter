WITH SOURCE_EVENTS
  AS ( SELECT E.CUSTOMER_ID,
              E.EVENT_ID,
              E.EVENT_TYPE,
              E.EVENT_TS,
              E.SOURCE_SYSTEM,
              E.PAYLOAD_HASH,
              ROW_NUMBER () OVER (PARTITION BY E.CUSTOMER_ID, E.EVENT_TYPE ORDER BY E.EVENT_TS DESC, E.INGESTED_TS DESC,
              E.EVENT_ID DESC) AS RN
         FROM CUSTOMER_EVENT E
        WHERE E.BATCH_ID = 20260520
          AND E.EVENT_TYPE IN ('LOGIN', 'PURCHASE', 'REFUND', 'SUPPORT')
          AND EXISTS (SELECT 1 FROM CUSTOMER C WHERE C.CUSTOMER_ID = E.CUSTOMER_ID AND C.IS_ACTIVE = 1) ),
     EVENT_ROLLUP
...
SELECT C.CUSTOMER_ID,
 ...
 WHERE C.IS_ACTIVE = 1
   AND (S.SCORE_CODE IN ('A', 'B') OR C.CUSTOMER_ID IN (SELECT P.CUSTOMER_ID
                                                                                      FROM CUSTOMER_PREFERENCE P
                                                                                     WHERE P.PREFERENCE_CODE =
                                                                                           'PRIORITY'
                                                                                       AND P.IS_ENABLED = 1))
...
MERGE INTO TGT_CUSTOMER_EVENT_SCORE T
 USING (
  SELECT C.CUSTOMER_ID,
         ...
         S.LAST_EVENT_TS,
         S.SCORE_CODE,
         ROW_NUMBER () OVER (PARTITION BY C.CUSTOMER_ID ORDER BY S.LAST_EVENT_TS DESC, C.UPDATED_AT DESC, C.CUSTOMER_ID
         ASC) AS RN
    FROM CUSTOMER C
    INNER JOIN CUSTOMER_SCORE S ON C.CUSTOMER_ID = S.CUSTOMER_ID
   WHERE C.IS_ACTIVE = 1
     AND (S.SCORE_CODE IN ('A', 'B', 'C') OR EXISTS (SELECT 1
   ...
   AND S.RN = 1)
  WHEN MATCHED
   AND ( COALESCE(T.CUSTOMER_NAME, '') <> COALESCE(S.CUSTOMER_NAME, '')
    ...
    OR COALESCE(T.SCORE_CODE, '') <> COALESCE(S.SCORE_CODE, ''))
  THEN
UPDATE
