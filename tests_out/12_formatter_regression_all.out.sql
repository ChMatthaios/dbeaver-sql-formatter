...
SELECT C.CUSTOMER_ID,
 ...
 USING ( SELECT C.CUSTOMER_ID,
           ...
           FROM CUSTOMER C
          INNER JOIN CUSTOMER_TOTALS CT ON C.CUSTOMER_ID = CT.CUSTOMER_ID
          WHERE C.IS_ACTIVE = 1 AND CT.ORDER_COUNT > 0 ) S
...
CREATE OR REPLACE PROCEDURE SP_REFRESH_CUSTOMER_STATUS (IN P_CUSTOMER_ID INTEGER) LANGUAGE SQL BEGIN UPDATE CUSTOMER SET STATUS_CODE = 'ACTIVE', UPDATED_AT = CURRENT TIMESTAMP, UPDATED_BY = CURRENT USER WHERE CUSTOMER_ID = P_CUSTOMER_ID WITH NC;

END;

CREATE OR REPLACE PROCEDURE SP_LOAD_CUSTOMER_SUMMARY (IN P_BATCH_ID INTEGER) LANGUAGE SQL BEGIN INSERT INTO CUSTOMER_SUMMARY (CUSTOMER_ID, ORDER_COUNT, TOTAL_AMOUNT, CREATED_AT, CREATED_BY) SELECT O.CUSTOMER_ID, COUNT (*) AS ORDER_COUNT, SUM (O.ORDER_TOTAL) AS TOTAL_AMOUNT, CURRENT TIMESTAMP AS CREATED_AT, CURRENT USER AS CREATED_BY FROM ORDER_HEADER O WHERE O.BATCH_ID = P_BATCH_ID AND O.ORDER_STATUS = 'PAID' GROUP BY O.CUSTOMER_ID HAVING SUM (O.ORDER_TOTAL) > 0 WITH NC;
...
CREATE OR REPLACE PROCEDURE SP_MERGE_CUSTOMER_EVENT_SCORE (IN P_BATCH_ID INTEGER, IN P_FORCE_REFRESH SMALLINT) LANGUAGE SQL BEGIN DECLARE V_STARTED_AT TIMESTAMP;
...
CREATE OR REPLACE FUNCTION FN_CUSTOMER_STATUS (P_CUSTOMER_ID INTEGER) RETURNS VARCHAR (20) LANGUAGE SQL BEGIN RETURN (SELECT STATUS_CODE FROM CUSTOMER WHERE CUSTOMER_ID = P_CUSTOMER_ID FETCH FIRST 1 ROW ONLY);

END;

CREATE OR REPLACE FUNCTION FN_CUSTOMER_SEGMENT (P_CUSTOMER_ID INTEGER) RETURNS VARCHAR (20) LANGUAGE SQL BEGIN RETURN (SELECT CASE WHEN SUM (O.ORDER_TOTAL) >= 10000 THEN 'VIP' WHEN SUM (O.ORDER_TOTAL) >= 5000 THEN 'GOLD' WHEN SUM (O.ORDER_TOTAL) >= 1000 THEN 'SILVER' ELSE 'STANDARD' END AS SEGMENT_CODE FROM ORDER_HEADER O WHERE O.CUSTOMER_ID = P_CUSTOMER_ID AND O.ORDER_STATUS = 'PAID' WITH UR);

END;

CREATE OR REPLACE FUNCTION FN_CUSTOMER_RISK_SCORE (P_CUSTOMER_ID INTEGER, P_AS_OF_DATE DATE) RETURNS DECIMAL (10, 2) LANGUAGE SQL BEGIN RETURN (WITH BASE_EVENTS AS (SELECT E.CUSTOMER_ID, E.EVENT_TYPE, E.EVENT_TS, E.RISK_WEIGHT, ROW_NUMBER () OVER (PARTITION BY E.CUSTOMER_ID, E.EVENT_TYPE ORDER BY E.EVENT_TS DESC, E.EVENT_ID DESC) AS RN FROM CUSTOMER_EVENT E WHERE E.CUSTOMER_ID = P_CUSTOMER_ID AND DATE (E.EVENT_TS) <= P_AS_OF_DATE) , RISK_ROLLUP AS (SELECT B.CUSTOMER_ID, SUM (CASE WHEN B.EVENT_TYPE = 'REFUND' THEN B.RISK_WEIGHT ELSE 0 END) AS REFUND_RISK, SUM (CASE WHEN B.EVENT_TYPE = 'SUPPORT' THEN B.RISK_WEIGHT ELSE 0 END) AS SUPPORT_RISK, SUM (B.RISK_WEIGHT) AS TOTAL_RISK FROM BASE_EVENTS B WHERE B.RN = 1 GROUP BY B.CUSTOMER_ID) SELECT CASE WHEN R.TOTAL_RISK IS NULL THEN 0 WHEN R.TOTAL_RISK > 100 THEN 100 ELSE R.TOTAL_RISK END AS RISK_SCORE FROM RISK_ROLLUP R WITH UR);
